...
async def transcribe_audio(
    ...
    try:
        ...
        if not speaker_segments:
            # Fallback a transcripción simple si no hay hablantes detectados
            result = await model_manager.transcribe(
                audio_path=tmp_path,
                language=language if language else "auto",
                prompt=prompt,
        ...
        with _tmp.TemporaryDirectory() as tmpdir:
            ...
            await asyncio.get_running_loop().run_in_executor(
                None, lambda: __import__("audio_utils").convert_to_wav(tmp_path, wav_path)
            )
        ...
        detected_lang = segments_output[0].get("language", language) if segments_output else language
